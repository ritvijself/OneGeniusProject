...
const DeviceBrowserChart = ({
  ...
  const fetchDistributionData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const apiUrl = `${apibaseurl}/api/${SquareBox.apiurl}/${
        SquareBox.url
      }?t=${Date.now()}`;
      ...
      if (Array.isArray(result) && result.length > 0) {
        ...
        chartData.datasets = months.map((month, index) => {
          ...
          return {
            ...
            data: data,
            backgroundColor:
              monthColors[index % monthColors.length].backgroundColor,
            borderColor: monthColors[index % monthColors.length].borderColor,
            borderWidth: 0,
          };
        });
      } else {
  ...
  }, [propertyId, startDate, endDate, SquareBox?.apiurl, token, apibaseurl]);
  ...
  return (
    <div className="card shadow-sm p-3 h-100">
      ...
      <div style={{ height: "200px", minWidth: "300px" }}>
